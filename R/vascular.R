...
vascular <- function(all_cells, params, layers, center){
  ...
  #\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\
  # ADDING A PROPERTY REGARDING A CENTRAL XYLEM:
  if(length(params$value[params$name == "central_xylem" & params$type == "max_size"]) > 0){
  ...
  if(plant_type == 2){ # DICOT
    ...
    if(n_xylem_files == 1){
      ...
      # r= max(all_cells$radius[all_cells$type == "stele"])
      # - (params$value[params$type == "cell_diameter" & params$name == "stele"])*1.5
      # - (params$value[params$type == "max_size" & params$name == "xylem"])/2
      #!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!
      fraction = 0.7
      r= (params$value[params$type == "layer_diameter" & params$name == "stele"])/2* fraction
      #!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!
      xyl <- data.frame(r = r,
                        d = params$value[params$type == "max_size" & params$name == "xylem"])
    }
    ...
    #\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\
    # DEFINING THE COORDINATES FOR A CENTRAL XYLEM:
    if(n_central_xylem_files == 1){ # One central xylem in the center of the stele
      r <- 0
      xyl_central <- data.frame(r = r,
                                d = params$value[params$type == "max_size" & params$name == "central_xylem"])
    } else if (n_central_xylem_files > 1) {
      r= max(all_cells$radius[all_cells$type == "stele"])
      - (params$value[params$type == "cell_diameter" & params$name == "stele"])*1.5
      - (params$value[params$type == "max_size" & params$name == "central_xylem"])/2
      xyl_central <- data.frame(r = r,
                                d = params$value[params$type == "max_size" & params$name == "central_xylem"])
    }
    #/////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////
    ...
    #\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\
    # ADDING THE NEW CENTRAL XYLEM CELL:
    if (n_central_xylem_files > 0) {
      x <- center + (xyl_central$r[1] * cos(0))
      y <- center + (xyl_central$r[1] * sin(0))
      all_central_xylem <- data.frame(x = x,
                                      y = y,
                                      d = xyl_central$d[1],
                                      angle = 0,
                                      id_group = i+1)
    ...
    #\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\
    # DEFINING THE CIRCULAR FRONTIERS OF THE CENTRAL XYLEM:
    if (n_central_xylem_files > 0) {
      x_cir <- seq(-0.95,0.95,0.95/4)
      y_p <- sqrt(1-x_cir^2)
      y_m <- -sqrt(1-x_cir^2)
    ...
    # We define the fraction of the radius of the stele where phloem vessels should be located:
    fraction = 0.7
    # We define the positions of the center of a phloem cell within the stele:
    phl <- data.frame(r = max(all_cells$radius[all_cells$type == "stele"])*fraction - (params$value[params$type == "cell_diameter" & params$name == "stele"])/2,
                      d = params$value[params$type == "cell_diameter" & params$name == "stele"])

    # We cover the position of each phloem cell according to the position of the protoxylem cells:
    ...
    for(angle in angle_seq_ph){
      ...
      all_cells <- all_cells %>%
        mutate(type = as.character(type)) %>%
        # We compute the distance of each cell from the phloem circle:
        mutate(dist_phl = sqrt((x-x1)^2 + (y-y1)^2)) %>%
      ...
      # COMPANION CELL TRANSFORMATION: We replace the nearest stele cell by a first companion cell:
      all_cells <- all_cells %>%
        mutate(type = as.character(type)) %>%
        # We now attribute a ridiculous value of 100 mm to dist_phl for any cell outside the stele
        # and for any stele cell located closer to the root center than the phloem circle:
        mutate(dist_phl = ifelse(type == "stele"
                                 & sqrt((x-center)^2 + (y-center)^2) > 1.1*sqrt((x1-center)^2 + (y1-center)^2),
                                 dist_phl, # If True, we keep the original value of dist_phl
                                 100)) %>% # If False, we replace dist_phl by 100
        # We replace the stele type if the distance to the phloem circle is the lowest one:
        mutate(type = ifelse(dist_phl == min(dist_phl),
      ...
      # COMPANION CELL TRANSFORMATION: We replace the nearest stele cell by a second companion cell:
      all_cells <- all_cells %>%
        mutate(type = as.character(type)) %>%
        # We now attribute a ridiculous value of 100 mm to dist_phl for any cell outside the stele
        # and for any stele cell located closer to the root center than the phloem circle:
    ...
    # Eventually, we remove the intermediary variable dist_phl:
    all_cells <- all_cells %>% select(-dist_phl, -dist_protoxyl)

    # ADJUSTING RADIUS OF PHLOEM CELLS:
